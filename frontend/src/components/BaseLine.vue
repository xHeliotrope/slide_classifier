    ...
    <v-content>
      ...
      >
        ...
        >
          <v-col class="text-center">
				    Classifications
						<MyModal v-bind:dialog="myModalDialog" v-bind:coords="coords" v-bind:filename="currentSelection"></MyModal>
          </v-col>
        </v-row>
      </v-container>
    </v-content>
      ...
      initSeadragon: function() {
        ...
        if(!this.viewer) {
          ...
          this.viewer = new OpenSeadragon(this.getOptions(tileSources));
              ...
              that.coords = rect;
              that.myModalDialog = true;
						}
          })
        this.viewer.addHandler('open', function(slideData){
          that.currentSelection = slideData.source.split('/')[3];
        })
        }
      },
